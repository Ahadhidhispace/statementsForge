...
<div class="relative max-h-fit transition-all duration-500 ease-out">
	<input
		on:focus={(e) => {
			field = 'ref';
			console.log(`I am a uuid from Ref component: ${uuid}`);
		}}
		on:blur={(e) => {
			field = '';
		...
		type="text"
		class="peer block h-[38px] w-[100%] rounded-md bg-white/80 ring-[0.05em] ring-black/20 focus:ring-0 px-2 pr-1 py-3 pt-6 text-xs font-bold leading-tight text-black/50 placeholder:text-black/30 placeholder:font-semibold focus:placeholder:opacity-20 border-none focus:border-none pb-[0.700rem] focus:pb-[0.700rem] pt-[1.340rem] focus:pt-[1.340rem] focus:text-black/70 {error
	...
	<label
		...
		class="pointer-events-none flex items-center absolute left-0 top-0 origin-[0_0] border-none px-2 py-5 text-black/30 transition-all duration-200 ease-out -translate-y-4 text-xs peer-focus:-translate-y-4 peer-focus:translate-x-[0.15rem] scale-[0.85] peer-focus:scale-[0.85] {error
