<script>
	...
	export let inputWidth = 'min-w-[73%]'
	...
	function sayHello(value) {
		dispatch('dateChanged', {
			content: 'I am from child component',
			dateVal: value
		});
	}
</script>
...
<div
	class="relative max-h-fit {inputWidth === 'min-w-[73%]' ? 'min-w-[73%]':'min-w-[94%]'} {isDisabled === true
		? 'opacity-50'
...
>
	<input
		...
		on:focus={(e) => {
			...
			console.log(`I am a uuid from dates component: ${uuid}`);
		}}
		on:blur={(e) => {
			field = '';
		...
		on:change={(e) => {
			...
			// 	// $transactions[$activeRecordID].amount = 0;
			// }
			// console.log(e);
			sayHello(e.currentTarget.value);
		}}
		id={identifier}
		bind:value={inputValue}
		type="date"
		class="peer block h-[38px] w-[100%] rounded-md bg-white/80 ring-[0.05em] ring-black/20 focus:ring-0 px-4 pr-2 py-3 test:pt-6 text-xs font-bold leading-tight text-black/50 placeholder:text-black/30 placeholder:font-semibold focus:placeholder:opacity-20 border-none focus:border-none pb-[0.700rem] focus:pb-[0.700rem] pt-[1.340rem] focus:pt-[1.340rem] focus:text-black/70 {error
